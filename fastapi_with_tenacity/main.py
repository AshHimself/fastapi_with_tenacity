...
def get_users(page: int = 1, page_size: int = 3):
    ...
    users = [
        generate_fake_user(user_id)
        for user_id in range(start + 1, min(end + 1, total_users + 1))
    ]
    return users
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if len(request_timestamps[client_ip]) >= DAILY_REQUEST_LIMIT:
            ...
            request_timestamps[client_ip] = [
                ts for ts in request_timestamps[client_ip] if current_time - ts < 86400
            ]
            if len(request_timestamps[client_ip]) >= DAILY_REQUEST_LIMIT:
                return JSONResponse(
                    status_code=429,
        ...
        if len(request_timestamps[client_ip]) > 0:
            if (
                current_time - request_timestamps[client_ip][-1]
        ...
        try:
            ...
            return JSONResponse(
                status_code=429,
                content={
                    "detail": "Concurrency limit reached. Please try again later."
                },
            )
